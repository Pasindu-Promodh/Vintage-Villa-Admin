...
const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  // Fix for handling multiple images in the conversion process
const handleImmediateConversion = async (imagesToConvert: ImageWithTags[]) => {
  setConverting(true);
  setConversionProgress(0);
  setConvertedCount(0);
  setTotalToConvert(imagesToConvert.length);

  // Create a working copy of the images array that we'll update during conversion
  let workingImages = [...imagesToConvert];
  ...
  for (let i = 0; i < workingImages.length; i++) {
    try {
      ...
      // Convert to WebP
      const webpFile = await convertToWebP(image.file) as File;
      
      // Create thumbnail
      const thumbnailFile = await createThumbnail(image.file);
      
      // Update our working copy with conversion results
      workingImages[i] = {
        ...image,
        convertedFile: webpFile,
      ...
      // Update React state with the entire working array
      setImages([...workingImages]);
      
      // Calculate new total converted size
      const newTotalConvertedSize = workingImages.reduce(
        (sum, img) => sum + (img.convertedSize || img.originalSize) + (img.thumbnailSize || 0),
        0
      );
      setTotalConvertedSize(newTotalConvertedSize);
    } catch (error) {
      console.error("Conversion failed for ", workingImages[i].file.name, error);
      
      // Mark as failed conversion but keep original
      workingImages[i] = {
        ...workingImages[i],
        isConverted: false
      };
  ...
  const originalTotalSize = imagesToConvert.reduce(
    (sum, img) => sum + img.originalSize,
    0
  );
  
  const finalImages = [...workingImages]; // Use our final working copy
  const newTotalSize = finalImages.reduce(
    (sum, img) => sum + (img.convertedSize || img.originalSize) + (img.thumbnailSize || 0),
    0
  );
  
  const savedBytes = originalTotalSize - newTotalSize;
  const savingsPercentage = originalTotalSize > 0 ? 
    Math.round((savedBytes / originalTotalSize) * 100) : 0;
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      // Release any preview URLs from a previous selection.
      ...
      // Start immediate conversion
      enqueueSnackbar("Converting images to WebP format...", { variant: "info" });
      handleImmediateConversion(newImages);
    }
  };
  ...
  const uploadPhotos = async () => {
    ...
    try {
      ...
      enqueueSnackbar(`Starting upload of ${images.length} image(s) with thumbnails...`, { variant: "info" });
      ...
      const uploadPromises = images.map(
        (image, index) => {
          ...
          const file = image.convertedFile || image.file;
          const thumbnailFile = image.thumbnailFile;
          const tags = image.tags;

          // Unique object keys so two files with the same name never
          // overwrite each other in Storage.
          const uid =
            typeof crypto !== "undefined" && "randomUUID" in crypto
              ? crypto.randomUUID()
              : `${currentDate}-${index}-${Math.random().toString(36).slice(2)}`;
          const safe = (name: string) => name.replace(/[^\w.\-]+/g, "_");
          const photoPath = `photos/${uid}_${safe(file.name)}`;
          const thumbPath = thumbnailFile
            ? `thumbnails/${uid}_${safe(thumbnailFile.name)}`
            : null;
            ...
            try {
              ...
              uploadTask.on(
                ...
                (snapshot) => {
                  ...
                  setUploadProgress(
                    Math.round((uploadedBytes / totalConvertedSize) * 100)
                  );
                },
                (error) => {
                  ...
                  try {
                    ...
                    await addDoc(collection(db, "photos"), {
                      ...
                      originalFilename:
                        ...
                        getOriginalExtension(image.file),
      ...
      enqueueSnackbar(`Upload complete! ${images.length} image${
          images.length !== 1 ? "s" : ""
  ...
  // Calculate stats for all images
  const calculateStats = () => {
    const originalTotal = images.reduce((total, img) => total + img.originalSize, 0);
    const convertedTotal = images.reduce((total, img) => total + (img.convertedSize || img.originalSize), 0);
    const thumbnailTotal = images.reduce((total, img) => total + (img.thumbnailSize || 0), 0);
    const totalAfterConversion = convertedTotal + thumbnailTotal;
    
    const savedBytes = originalTotal - totalAfterConversion;
    const savingsPercentage = originalTotal > 0 ? Math.round((savedBytes / originalTotal) * 100) : 0;
    
    return {
      originalTotal,
      convertedTotal,
      thumbnailTotal,
      totalAfterConversion,
      savedBytes,
      savingsPercentage
    };
  };
    ...
    >
      ...
      {images.length > 0 && !converting && (
        <Box mt={isMobile ? 1 : 2} mb={isMobile ? 1 : 2}>
          <Paper elevation={2} sx={{ p: isMobile ? 1 : 2, bgcolor: '#f5f5f5' }}>
            <Typography variant="h6" gutterBottom>Storage Stats</Typography>
            <Grid container spacing={1}>
              <Grid item xs={6}>
                <Typography variant="body2">Original Size:</Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="body2" fontWeight="bold">{formatFileSize(stats.originalTotal)}</Typography>
              </Grid>
              
              <Grid item xs={6}>
                <Typography variant="body2">Size After Conversion:</Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="body2" fontWeight="bold">
                  {formatFileSize(stats.totalAfterConversion)}
                  {stats.savingsPercentage > 0 && (
                    <span style={{ color: 'green' }}> (-{stats.savingsPercentage}%)</span>
                  )}
                </Typography>
              </Grid>
              ...
              {stats.savedBytes > 0 && (
                <>
                  <Grid item xs={6}>
                    <Typography variant="body2">Storage Saved:</Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="body2" fontWeight="bold" color="green">
                      {formatFileSize(stats.savedBytes)}
                    </Typography>
                  </Grid>
                </>
              )}
                  ...
                  <Grid item xs={12} sm={9}>
                    ...
                    <Box mt={0.5}>
                      ...
                      {image.isConverted ? (
                        <>
                          <Typography variant="body2">
                            After WebP conversion: {formatFileSize(image.convertedSize || 0)}
                            {image.originalSize > (image.convertedSize || 0) && (
                              <span style={{ color: 'green' }}> 
                                (-{Math.round(((image.originalSize - (image.convertedSize || 0)) / image.originalSize) * 100)}%)
                              </span>
                            )}
                          </Typography>
                          <Typography variant="body2">
                            Thumbnail: {formatFileSize(image.thumbnailSize || 0)}
                          </Typography>
                        </>
                      ) : !converting && (
                        <Typography variant="body2" color="error">
                          Conversion failed, will use original
                        </Typography>
                      )}
                    </Box>
                    ...
                    <Box mt={0.5} display="flex" justifyContent="flex-end">
                      <IconButton
                        onClick={() => {
                          ...
                          // Recalculate stats
                          const stats = calculateStats();
                          setTotalConvertedSize(stats.totalAfterConversion);
                        }}
                        color="error"
      ...
      <Button
        ...
        variant="contained"
        fullWidth={isMobile}
        disabled={uploading || images.length === 0 || converting}
        sx={{ mt: isMobile ? 1 : 2 }}
      >
